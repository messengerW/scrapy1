...
class PlayerTurnSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if datas['data']:                           # 判断是否为空，不为空则提取出数据
            ...
            # 用正则表达式提取出当前爬取页面的轮次
            turn = re.search(r"stageName%5D=(\d+)", response.url).group(1)
