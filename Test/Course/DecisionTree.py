...
def Test1():
    ...
    # 使用全部的原始特征构建决策树
    # lensesTree1 = createTree(datalist, featlabels)
    # print(lensesTree1)
    # 画图可视化展现
    # dtPlot.createPlot(lensesTree1)

    # 使用排序后的特征子集构建决策树
    k, sed_list, df1, df2 = exam1.getStandardizedEDList(df)
    as_list = exam1.getAdjacentSample(k, sed_list)
    dw_list = exam1.calculateDiff(as_list, df1, df2)
    result = exam1.sortFeature(df1, dw_list)
    print(result)
    # lensesTree2 = createTree(datalist, featlabels)
    # dtPlot.createPlot(lensesTree2)
